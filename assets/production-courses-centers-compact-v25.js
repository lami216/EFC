...
const waitUntil=async(check,timeout=15000)=>{const start=Date.now();while(!check()){if(Date.now()-start>timeout)throw new Error('Courses/centers compact v25 timed out.');await new Promise(resolve=>setTimeout(resolve,20));}};
await waitUntil(()=>window.EFC_COURSES_CENTERS_REDESIGN_V23?.ready);
...
window.EFC_COURSES_CENTERS_COMPACT_V25=Object.freeze({
  ...
  noStretch:true,fixedCardWidths:true,adaptiveCardHeights:true,longNamesWrapInsideCards:true,mainUntouched:true
});
})();
