...
function renderStudentsBaseV13(){
  ...
  shell(`${pageTitle('الملفات','البحث عن طالب','ابحث عن الطالب واعرض معلوماته الأساسية والدورة دون إقحام التفاصيل المالية في قائمة البحث.')}<div class="card filters student-search-filters-v13"><input class="input" id="studentSearchV13" placeholder="الاسم، الهاتف أو رقم السجل" autocomplete="off"><select id="studentBranchV13">${opts(branches,x=>x.id,x=>x.name,'كل الفروع')}</select><select id="studentSpecV13">${opts(specialties,x=>x.id,x=>x.name,'كل الدورات')}</select></div><div id="studentsTableV13"></div>`);
...
function renderPeriodBaseV13(){
  ...
  function draw(){const q=document.getElementById('periodSearchV13').value.trim().toLowerCase(),from=document.getElementById('periodFromV13').value,to=document.getElementById('periodToV13').value||current,branch=document.getElementById('periodBranchV13').value,specialty=document.getElementById('periodSpecV13').value,state=document.getElementById('periodStateV13').value,base=student=>identity(student,q)&&(!branch||student.branch===branch)&&(!specialty||student.specialty===specialty)&&(!state||financialStatus(student)===state);let columns=[],records=[];
