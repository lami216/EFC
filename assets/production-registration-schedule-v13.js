...
function scheduleMarkup(){return`<section class="registration-schedule-card-v13" aria-label="جدول الطالب"><div class="schedule-title-v13"><div><small>تنظيم الحصص</small><h2>جدول الطالب الأسبوعي</h2></div><span>الأيام والوقت</span></div><p class="schedule-top-note-v13">${esc(noteTop)}</p><div class="schedule-table-wrap-v13"><table class="schedule-table-v13"><thead><tr><th class="schedule-course-head-v13">الدورة</th>${DAYS.map(day=>`<th><b>${day.ar}</b><small>${day.fr}</small></th>`).join('')}</tr></thead><tbody><tr class="schedule-time-row-v13"><th>الوقت</th>${DAYS.map(day=>`<td><select class="schedule-hour-select-v13" data-day-time="${day.key}" aria-label="ساعة ${day.ar}">${hourOptions()}</select></td>`).join('')}</tr><tr class="schedule-course-row-v13"><th id="scheduleCourseNameV13">اختر الدورة</th>${DAYS.map(day=>`<td><label class="schedule-check-v13" title="${day.ar}"><input type="checkbox" data-day-check="${day.key}" disabled><span></span></label></td>`).join('')}</tr></tbody></table></div><div class="schedule-notes-v13"><p>${esc(noteOne)}</p></div></section>`;}
