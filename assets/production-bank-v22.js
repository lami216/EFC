...
function bankPeriodReportBody(model){
  return`${bankPeriodReportHeader()}<div class="bank-report-title-v23">تقرير البنك</div><div class="bank-report-meta-v23"><span>تاريخ الإصدار: <b>${showDate(model.createdDate)}</b></span><span>حركات الدخل: <b>${model.incomeCount}</b></span><span>حركات الصرف: <b>${model.expenseCount}</b></span></div><div class="bank-report-period-v23"><small>الفترة</small><b>${esc(model.label)}</b></div><div class="bank-report-kpis-v23"><div class="bank-report-kpi-v23 income"><small>إجمالي الدخل</small><b>${cash(model.income)}</b><em>${model.incomeCount} حركة دخل</em></div><div class="bank-report-kpi-v23 expense"><small>إجمالي الصرف</small><b>${cash(model.expense)}</b><em>${model.expenseCount} حركة صرف</em></div><div class="bank-report-kpi-v23 balance"><small>المتبقي</small><b>${cash(model.balance)}</b><em>الدخل ناقص الصرف</em></div></div><div class="bank-report-count-v23">هذا التقرير يعرض حركات البنك المسجلة داخل الفترة المحددة فقط.</div>`;
}
...
function drawBankPeriod(){
  ...
  const incomeEl=document.getElementById('bankIncomeTotalV23'),expenseEl=document.getElementById('bankExpenseTotalV23'),balanceEl=document.getElementById('bankBalanceTotalV23'),incomeCountEl=document.getElementById('bankIncomeCountV23'),expenseCountEl=document.getElementById('bankExpenseCountV23'),summary=document.getElementById('bankPeriodSummaryV23'),table=document.getElementById('bankRowsV23');
...
async function boot(){
  ...
  window.EFC_BANK_V22=Object.freeze({ready:true,bankIndependentFromFinance:true,bankEntriesEditableAndDeletable:true,bankStatementFixedHeight:true,bankUsesIncomeAndExpenseLabels:true,bankPeriodFiltersDailyWeeklyMonthlyYearly:true,bankPeriodReport:true,bankPeriodSummaryColors:true,bankIncomeExpenseCounts:true,bankLedgerStudentSearchTableStyle:true,bankReceiptSharedEmbeddedLogo:true,bankBackupRestore:true,bankOwnReceiptSequence:true,noFinanceStreamIntegration:true});
}
