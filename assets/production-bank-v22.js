...
function receiptBody(entry){
  const kind=entry.type==='out'?'مخرج':'دخل';
...
function renderBank(){
  ...
  shell(`<section class="bank-hero-v22">${icon}<h1>البنك</h1></section><div class="card bank-form-v22"><label>نوع الحركة<select id="bankTypeV22" ${editable?'':'disabled'}><option value="in" ${editing?.type!=='out'?'selected':''}>دخل</option><option value="out" ${editing?.type==='out'?'selected':''}>مخرج</option></select></label><label>المبلغ<input class="input" id="bankAmountV22" type="number" min="1" value="${editing?Number(editing.amount||0):''}" ${editable?'':'disabled'}></label><label>التاريخ<input class="input" id="bankDateV22" type="date" value="${esc(editing?.date||today())}" ${editable?'':'disabled'}></label><label>البيان / الملاحظة<textarea id="bankStatementV22" ${editable?'':'disabled'} placeholder="اكتب سبب الحركة أو ملاحظتها">${esc(editing?.statement||'')}</textarea></label><button class="button" id="bankSaveV22" type="button" ${editable?'':'disabled'}>${editing?'حفظ التعديل':'تسجيل الحركة'}</button>${editing?'<div class="bank-edit-note-v22">أنت تعدل حركة موجودة. رقم الروسي سيبقى كما هو.</div>':''}</div><div class="bank-kpis-v22"><div class="card"><small>إجمالي الدخل</small><b>${cash(income)}</b></div><div class="card"><small>إجمالي المخرج</small><b>${cash(outcome)}</b></div><div class="card balance"><small>الرصيد</small><b>${cash(balance)}</b></div></div><div class="card bank-table-v22">${rows.length?`<div class="table-wrap"><table><thead><tr><th>رقم الروسي</th><th>النوع</th><th>المبلغ</th><th>البيان</th><th>التاريخ</th><th>الوقت</th><th>إجراء</th></tr></thead><tbody>${rows.map(entry=>`<tr data-bank-id="${esc(entry.id)}"><td>${receiptCode(entry)}</td><td class="${entry.type==='out'?'bank-type-out':'bank-type-in'}">${entry.type==='out'?'مخرج':'دخل'}</td><td>${cash(entry.amount)}</td><td>${esc(entry.statement)}</td><td>${showDate(entry.date)}</td><td>${esc(entry.time)}</td><td><div class="bank-actions-cell-v22"><button class="mini bank-receipt-v22" type="button">روسي</button><button class="mini bank-edit-v22" type="button" ${editable?'':'disabled'}>تعديل</button><button class="mini bank-delete-v22" type="button" ${editable?'':'disabled'}>حذف</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="bank-empty-v22">لا توجد حركات بنك مسجلة بعد.</div>'}</div>`);
...
async function boot(){
  ...
  window.EFC_BANK_V22=Object.freeze({ready:true,bankIndependentFromFinance:true,bankEntriesEditableAndDeletable:true,bankReceiptSharedEmbeddedLogo:true,bankBackupRestore:true,bankOwnReceiptSequence:true,noFinanceStreamIntegration:true});
}
