...
function normalizeScope(value={}){
  const latest=Math.max(0,Number(value.latestIssued||value.latest||0)),sealed=Math.max(0,Number(value.sealedThrough||0));
  const latestIssued=Math.max(latest,sealed?sealed+1:0);
  return{latestIssued,sealedThrough:Math.min(sealed,Math.max(0,latestIssued-1)),updatedAt:Math.max(0,Number(value.updatedAt||0))};
}
...
function mergeStates(aRaw,bRaw){
  ...
  Object.entries(b.scopes).forEach(([key,value])=>{
    ...
    if(!current||Number(value.updatedAt||0)>Number(current.updatedAt||0))scopes[key]=value;
    else if(Number(value.updatedAt||0)===Number(current.updatedAt||0))scopes[key]={latestIssued:Math.max(current.latestIssued,value.latestIssued),sealedThrough:Math.max(current.sealedThrough,value.sealedThrough),updatedAt:current.updatedAt};
  });
...
function ensureScope(branch,specialty){
  const key=scopeKey(branch,specialty);let entry=state.scopes[key];
  if(!entry){
    const legacy=int(sequenceBase.identitySnapshot?.()?.registrationLastByScope?.[key])||0,regs=activeRegs(branch,specialty),activeMax=regs.length?Math.max(...regs):0,latest=Math.max(legacy,activeMax);
    entry={latestIssued:latest,sealedThrough:Math.max(0,latest-1),updatedAt:0};state.scopes[key]=entry;
  }
  const regs=activeRegs(branch,specialty),activeMax=regs.length?Math.max(...regs):0;
  if(activeMax>entry.latestIssued){entry.sealedThrough=Math.max(entry.sealedThrough,activeMax-1);entry.latestIssued=activeMax;entry.updatedAt=now();state.updatedAt=entry.updatedAt;writeLocal(false);}
  return entry;
}
function seedFromLegacyAndStudents(){
  const legacy=sequenceBase.identitySnapshot?.()?.registrationLastByScope||{};
  Object.entries(legacy).forEach(([key,value])=>{if(state.scopes[key])return;const latest=int(value)||0;state.scopes[key]={latestIssued:latest,sealedThrough:Math.max(0,latest-1),updatedAt:0};});
  (typeof students!=='undefined'?students:[]).forEach(student=>{
    const reg=int(student?.reg);if(!reg)return;const key=scopeKey(student.branch,student.specialty),entry=state.scopes[key]||{latestIssued:0,sealedThrough:0,updatedAt:0};
    if(reg>entry.latestIssued){entry.sealedThrough=Math.max(entry.sealedThrough,reg-1);entry.latestIssued=reg;entry.updatedAt=Math.max(entry.updatedAt,Number(student.updatedAt||0));state.scopes[key]=entry;}
  });
}
function previewRegistrationNumber(branch,specialty){
  ...
  const occupied=new Set(activeRegs(branch,specialty));return occupied.has(latest)?latest+1:latest;
}
function commitRegistrationNumber(branch,specialty,number){
  const value=int(number);if(!value)return null;const entry=ensureScope(branch,specialty),stamp=now();
  if(value>entry.latestIssued){entry.sealedThrough=Math.max(entry.sealedThrough,value-1);entry.latestIssued=value;}
  else if(value===entry.latestIssued){entry.latestIssued=value;}
  else if(value>entry.sealedThrough){entry.sealedThrough=Math.max(entry.sealedThrough,value);}
  entry.updatedAt=stamp;state.updatedAt=stamp;markChanged();return value;
}
function allocateRegistrationNumber(branch,specialty){return previewRegistrationNumber(branch,specialty);}
function noteRegistrationNumber(branch,specialty,number){return commitRegistrationNumber(branch,specialty,number);}
...
function monthlyCoverageMonth(student){
  if(!student||!B.isDynamicMonthly?.(student))return null;
...
function monthlyCoverageEnd(student){
  if(!student||!B.isDynamicMonthly?.(student)||!student.start)return'';
...
async function deleteStudentPermanently(student){
  ...
  window.EFC_CODES?.ensureStudentRecord?.(existing);const reusableRegistration=registrationNumberWillBeReusable(existing),paymentCount=Array.isArray(existing.payments)?existing.payments.length:0,paidAmount=B.paymentTotal?.(existing)||0,stamp=now();
  ...
  catch(error){if(index>=0)students.splice(index,0,existing);state.tombstones=state.tombstones.filter(item=>item!==tombstone&&!(item.id===tombstone?.id&&item.recordCode===tombstone?.recordCode&&item.deletedAt===stamp));writeLocal(true);throw error;}
...
function updateStudentRegistration(student,changes={}){
  ...
  const targetBranch=String(changes.branch??student.branch??''),targetSpecialty=String(changes.specialty??student.specialty??''),scopeChanged=targetBranch!==String(student.branch||'')||targetSpecialty!==String(student.specialty||'');
  ...
  try{
    ...
    commitRegistrationNumber(targetBranch,targetSpecialty,candidate);B.saveStudents?.();return result;
...
const next=Object.freeze({...B,updateStudentRegistration,monthlyCoverageMonth,monthlyCoverageEnd,deleteStudentPermanently,registrationNumberWillBeReusable,studentLifecycleV20:true,automaticRegistrationNumberOnScopeChange:true,manualStudentDeletionTombstones:true,closedFiscalStudentDeletionBlocked:true});
window.EFC_DOMAIN_V13=next;
window.EFC_DOMAIN_V13_READY=Promise.resolve(next);
window.EFC_STUDENT_LIFECYCLE_DOMAIN_V20=Object.freeze({ready:true,version:VERSION,storageKey:STORAGE_KEY,registrationNumberPolicy:'successor-sealed-latest-reusable',latestNumberReusableUntilSuccessor:true,olderNumbersStayReserved:true,automaticNumberOnScopeChange:true,monthlyCoverageEnd,previewRegistrationNumber,registrationNumberWillBeReusable,deleteStudentPermanently,snapshot:()=>clone(state)});
})();