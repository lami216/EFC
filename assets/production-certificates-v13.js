...
function drawCertificateFinance(){
  ...
  root.innerHTML=`<div class="grid three breakdowns"><div class="card"><h3>حسب الفرع</h3>${breakdownHtml(row=>row.branchName||'—')}</div><div class="card"><h3>حسب الشهادة / الدورة</h3>${breakdownHtml(row=>row.specialtyName||'—')}</div><div class="card"><h3>حسب وسيلة الدفع</h3>${breakdownHtml(row=>row.method||'—')}</div></div>${table(['رقم الروسي','الطالب','الفرع','الدورة','الوسيلة','التاريخ','المبلغ'],tableRows)}`;
