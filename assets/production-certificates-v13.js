...
function certificateFinanceYears(){const current=Number(today().slice(0,4)),values=new Set([current]);state.certificateReceipts.forEach(receipt=>{const year=Number(String(receipt.date||'').slice(0,4));if(year>1900&&year<2200)values.add(year);});return[...values].sort((a,b)=>b-a);}
...
function ensureCertificateFinanceStyles(){
  if(document.getElementById('efc-certificate-finance-style-v41'))return;
  const style=document.createElement('style');style.id='efc-certificate-finance-style-v41';style.textContent=`
body.efc-certificates-workspace-v36 .cert-finance-v13 .cert-finance-controls-v13{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:10px 12px!important;margin:0 0 14px!important;padding:12px!important;border:1px solid #d7e5e0!important;border-radius:11px!important;background:#f8fcfa!important}
body.efc-certificates-workspace-v36 .cert-finance-v13 .cert-finance-controls-v13 label{min-width:0!important}
body.efc-certificates-workspace-v36 .cert-finance-v13 .cert-finance-controls-v13 input,body.efc-certificates-workspace-v36 .cert-finance-v13 .cert-finance-controls-v13 select{width:100%!important;min-width:0!important}
body.efc-certificates-workspace-v36 .cert-finance-summary-v41{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:14px!important;margin:0 0 16px!important}
body.efc-certificates-workspace-v36 .cert-finance-kpi-v41{min-height:112px!important;padding:18px 20px!important;border:1px solid #cfe0da!important;border-radius:14px!important;background:linear-gradient(135deg,#f8fffc,#edf9f4)!important;box-shadow:0 7px 20px rgba(15,91,70,.05)!important;display:flex!important;flex-direction:column!important;justify-content:center!important;gap:5px!important}
body.efc-certificates-workspace-v36 .cert-finance-kpi-v41 small{font-size:12px!important;font-weight:850!important;color:#4e6c62!important}
body.efc-certificates-workspace-v36 .cert-finance-kpi-v41 b{font-size:28px!important;line-height:1.1!important;font-weight:900!important;color:#073f35!important}
body.efc-certificates-workspace-v36 .cert-finance-kpi-v41 span{font-size:10.5px!important;font-weight:700!important;color:#6d817a!important}
body.efc-certificates-workspace-v36 .cert-finance-kpi-v41.is-total-v41{background:linear-gradient(135deg,#e9f8f2,#dff3eb)!important;border-color:#acd2c5!important}
body.efc-certificates-workspace-v36 .cert-finance-v13 .table-wrap{margin-top:0!important}
@media(max-width:980px){body.efc-certificates-workspace-v36 .cert-finance-v13 .cert-finance-controls-v13{grid-template-columns:repeat(2,minmax(0,1fr))!important}}
`;
...
function drawCertificateFinance(){
  ...
  const matchesFilters=receipt=>(!branch||certificateBranchKey(receipt)===branch)&&(!specialty||certificateSpecialtyKey(receipt)===specialty)&&(!method||String(receipt.method||'')===method);
  const allRows=[...state.certificateReceipts].filter(matchesFilters),rows=allRows.filter(receipt=>receipt.date>=range.from&&receipt.date<=range.to).sort((a,b)=>b.date.localeCompare(a.date)||String(b.time||'').localeCompare(String(a.time||''))||Number(b.timestamp||0)-Number(a.timestamp||0));
  const periodTotal=rows.reduce((sum,row)=>sum+Number(row.amount||0),0),overallTotal=allRows.reduce((sum,row)=>sum+Number(row.amount||0),0),periodTitle=modeValue==='daily'?'دخل اليوم':modeValue==='monthly'?'دخل الشهر':'دخل السنة';
  ...
  root.innerHTML=`<div class="cert-finance-summary-v41"><div class="cert-finance-kpi-v41"><small>${periodTitle}</small><b>${cash(periodTotal)}</b><span>${esc(range.label)}</span></div><div class="cert-finance-kpi-v41 is-total-v41"><small>الدخل العام</small><b>${cash(overallTotal)}</b><span>إجمالي دخل الشهادات ضمن الفلاتر الحالية</span></div></div>${table(['رقم الروسي','الطالب','الفرع','الدورة','الوسيلة','التاريخ','المبلغ'],tableRows)}`;
  bindCertificateFinanceRows();
}
function setHistoryMode(show){
  historyOpen=Boolean(show);const form=document.querySelector('.cert-form-v13'),finance=document.querySelector('.cert-finance-v13'),toggle=document.querySelector('.efc-cert-history-toggle-v36');
...
function renderCertificates(){
  ...
  ['certFinanceModeV13','certFinanceDayV13','certFinanceMonthV13','certFinanceYearV13','certFinanceBranchV13','certFinanceSpecialtyV13','certFinanceMethodV13'].forEach(id=>document.getElementById(id)?.addEventListener('change',drawCertificateFinance));
