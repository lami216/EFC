...
function receiptBody(receipt){
  ...
  return`${receiptHeader(receipt)}<div class="cert-section-title">إدارة الشهادات</div><div class="cert-meta"><span>تم الدفع في تاريخ: <b>${showDate(receipt.date)}</b></span><span>الفرع: <b>${esc(receipt.branchName||'—')}</b></span></div>${row("Nom de l’étudiant",receipt.studentName,'اسم الطالب')}${row('Nature de la Session',receipt.specialtyName,'الشهادة')}${row('Filière',receipt.branchName,'الفرع')}<div class="cert-pair">${half('N° Registre',reg,'رقم السجل','cert-reg')}${half('Montant',cash(receipt.amount),'المبلغ')}</div><div class="cert-methods">${receiptMethods(receipt.method)}</div>`;
}
...
function certificateManagerRows(from,to){
  const start=String(from||today()),end=String(to||start),min=start<=end?start:end,max=start<=end?end:start;
  return state.certificateReceipts.filter(receipt=>String(receipt.date||'')>=min&&String(receipt.date||'')<=max);
}
function certificateManagerReceiptModel(from,to,fundsLocation){
  const start=String(from||today()),end=String(to||start),min=start<=end?start:end,max=start<=end?end:start,rows=certificateManagerRows(min,max);
  return{from:min,to:max,count:rows.length,total:rows.reduce((sum,row)=>sum+Number(row.amount||0),0),fundsLocation:fundsLocation==='bank'?'البنك':'وسائل الدفع الإلكترونية',createdDate:today(),createdTime:nowTime()};
}
function certificateManagerReceiptCss(){return`
*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;margin:0;background:#eef1f0;color:#111715}.cert-manager-paper-v22{width:1040px;max-width:96vw;margin:18px auto;background:#fff;border:2px solid #293631;padding:12px 18px 20px;direction:ltr}.head12{display:grid;grid-template-columns:240px 1fr 150px;gap:14px;align-items:center;border-bottom:1px solid #b2b8b5;padding-bottom:6px}.contact12{display:grid;grid-template-columns:92px 1fr;gap:8px;align-items:center;direction:ltr;text-align:left}.contact12 img,.logoOnly12 img{width:82px;height:62px;object-fit:contain;display:block}.contactText12{display:grid;gap:1px}.contactText12>b{font-size:13px;white-space:nowrap}.socialLine12{display:flex;align-items:center;gap:5px;font-size:13px;font-weight:700;white-space:nowrap}.socialLine12.teacher12{font-size:10px;margin-top:2px}.socialLine12.teacher12 span:last-child{direction:rtl}.socialIcon12{width:14px;height:14px;display:inline-block;flex:0 0 14px}.socialIcon12 svg{width:100%;height:100%;fill:currentColor}.center12{text-align:center;direction:rtl}.title12{display:flex;direction:ltr;justify-content:center;align-items:baseline;gap:12px;white-space:nowrap;margin:0;font-size:27px}.official12{font-size:11px;font-weight:900;margin-top:3px}.tag12{font-size:11px;font-weight:800;margin-top:3px}.logoOnly12{height:66px;display:grid;place-items:center}.cert-manager-title-v22{text-align:center;direction:rtl;font-size:26px;font-weight:950;margin:12px 0 10px;color:#075844}.cert-manager-meta-v22{display:flex;justify-content:space-between;gap:20px;direction:ltr;padding:7px 0;border-bottom:1px solid #d9dedc}.cert-manager-meta-v22 span{direction:rtl}.cert-manager-grid-v22{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;direction:rtl;margin:18px 0}.cert-manager-box-v22{border:1px solid #ccd9d4;border-radius:10px;padding:15px;text-align:center;background:#fbfdfc}.cert-manager-box-v22 small{display:block;color:#64736e;font-size:10px;margin-bottom:7px}.cert-manager-box-v22 b{font-size:20px}.cert-manager-box-v22.total b{color:#075844;font-size:24px}.cert-manager-location-v22{direction:rtl;text-align:center;border:1px solid #d7dfdc;border-radius:10px;padding:13px;font-size:14px}.cert-manager-location-v22 b{font-size:18px;color:#7d2e2e}.cert-manager-actions-v22{width:1040px;max-width:96vw;margin:0 auto 18px;display:flex;direction:rtl;gap:8px}.cert-manager-actions-v22 button{border:0;border-radius:7px;padding:10px 17px;font:700 13px Tahoma;cursor:pointer}.cert-manager-print-v22{background:#155ea8;color:#fff}.cert-manager-save-v22{background:#159a55;color:#fff}@media(max-width:760px){.cert-manager-grid-v22{grid-template-columns:1fr}}@media print{body{background:#fff}.cert-manager-paper-v22{width:100%;max-width:none;margin:0;border:1px solid #222}.cert-manager-actions-v22{display:none}@page{size:landscape;margin:8mm}}
`;}
function certificateManagerReceiptHeader(){
  const img=`<img src="${logoUrl()}" alt="EFC">`;
  return`<div class="head12"><div class="contact12">${img}<div class="contactText12"><b>Tél: 48 02 84 84</b><div class="socialLine12">${whatsappIcon()}<span>32 09 86 89</span></div><div class="socialLine12 teacher12">${facebookIcon()}<span>الأستاذ محمد ديدي</span></div></div></div><div class="center12"><h1 class="title12"><span>Centre EFC</span><span>مركز</span></h1><div class="official12">للغات والمعلوماتية</div><div class="tag12">مالية الشهادات</div></div><div class="logoOnly12">${img}</div></div>`;
}
function certificateManagerReceiptBody(model){
  return`${certificateManagerReceiptHeader()}<div class="cert-manager-title-v22">روسي سحب مالية الشهادات</div><div class="cert-manager-meta-v22"><span>تاريخ الإصدار: <b>${showDate(model.createdDate)}</b></span><span>الفترة: <b>من ${showDate(model.from)} إلى ${showDate(model.to)}</b></span></div><div class="cert-manager-grid-v22"><div class="cert-manager-box-v22"><small>من تاريخ</small><b>${showDate(model.from)}</b></div><div class="cert-manager-box-v22"><small>إلى تاريخ</small><b>${showDate(model.to)}</b></div><div class="cert-manager-box-v22"><small>عدد الشهادات</small><b>${model.count}</b></div></div><div class="cert-manager-box-v22 total"><small>إجمالي مبلغ الشهادات</small><b>${cash(model.total)}</b></div><div class="cert-manager-location-v22">مكان المبلغ: <b>${esc(model.fundsLocation)}</b></div>`;
}
function certificateManagerReceiptDocument(model,actions=true,autoPrint=false){
  const data=JSON.stringify(model).replace(/</g,'\\u003c');
  return`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>روسي مالية الشهادات</title><style>${certificateManagerReceiptCss()}</style></head><body><div class="cert-manager-paper-v22">${certificateManagerReceiptBody(model)}</div>${actions?`<div class="cert-manager-actions-v22"><button class="cert-manager-print-v22" onclick="print()">طباعة</button><button class="cert-manager-save-v22" onclick="parent.EFC_SAVE_CERT_MANAGER_RECEIPT_PDF_V22(MANAGER_RECEIPT)">حفظ PDF</button></div>`:''}<script>const MANAGER_RECEIPT=${data};${autoPrint?'setTimeout(()=>print(),250);':''}<\/script></body></html>`;
}
function openCertificateManagerReceipt(model,autoPrint=false){
  const modal=document.createElement('div');modal.className='modal receipt-viewer-v13';modal.innerHTML='<div class="receipt-viewer-card-v13"><div class="receipt-viewer-head-v13"><b>روسي مالية الشهادات</b><button class="receipt-viewer-close-v13" type="button">×</button></div><iframe class="receipt-viewer-frame-v13" title="روسي مالية الشهادات"></iframe></div>';document.body.appendChild(modal);const frame=modal.querySelector('iframe'),close=()=>modal.remove();modal.querySelector('.receipt-viewer-close-v13').onclick=close;frame.srcdoc=certificateManagerReceiptDocument(model,true,autoPrint);return{close,frame};
}
async function saveCertificateManagerReceiptPdf(model){
  ...
  try{
    ...
    const fileName=`روسي-مالية-الشهادات-${model.from}-${model.to}.pdf`,buffer=pdf.output('arraybuffer');if(!invoke){pdf.save(fileName);return fileName;}const saved=await invoke('save_receipt_pdf',{fileName,dataBase64:arrayBufferToBase64(buffer)});if(saved)alert(`تم حفظ روسي مالية الشهادات:\n${saved}`);return saved;
  }catch(error){if(error?.name==='AbortError')return null;console.error('EFC certificate manager receipt PDF failed.',error);alert(String(error?.message||error||'تعذر حفظ روسي مالية الشهادات.'));return null;}finally{stage?.remove();}
}
function openCertificateManagerReceiptDialog(){
  const range=certificateManagerCurrentRange(),modal=document.createElement('div');modal.className='modal';modal.innerHTML=`<div class="modal-card narrow"><div class="modal-head"><div><p>مالية الشهادات</p><h2>إنشاء روسي المدير</h2><span>حدد الفترة ثم اختر أين يوجد المبلغ.</span></div><button class="x" type="button">×</button></div><form class="grid two" autocomplete="off"><label>من تاريخ<input class="input" name="from" type="date" value="${esc(range.from)}" required></label><label>إلى تاريخ<input class="input" name="to" type="date" value="${esc(range.to)}" required></label><label class="wide">أين يوجد المبلغ؟<select name="fundsLocation" required><option value="">اختر</option><option value="electronic">وسائل الدفع الإلكترونية</option><option value="bank">البنك</option></select></label><div class="wide modal-actions"><button type="button" class="button secondary cancel">إلغاء</button><button class="button">إنشاء الروسي</button></div></form></div>`;document.body.appendChild(modal);const close=()=>modal.remove();modal.querySelector('.x').onclick=close;modal.querySelector('.cancel').onclick=close;modal.querySelector('form').onsubmit=event=>{event.preventDefault();const data=new FormData(event.currentTarget),from=String(data.get('from')||''),to=String(data.get('to')||''),fundsLocation=String(data.get('fundsLocation')||'');if(!from||!to)return alert('حدد الفترة.');if(!fundsLocation)return alert('اختر أين يوجد المبلغ.');const model=certificateManagerReceiptModel(from,to,fundsLocation);if(!model.count)return alert('لا توجد شهادات في هذه الفترة.');close();openCertificateManagerReceipt(model);};
}
function drawCertificateFinance(){
  ...
  root.innerHTML=`<div class="grid three breakdowns"><div class="card"><h3>حسب الفرع</h3>${breakdownHtml(row=>row.branchName||'—')}</div><div class="card"><h3>حسب الشهادة</h3>${breakdownHtml(row=>row.specialtyName||'—')}</div><div class="card"><h3>حسب وسيلة الدفع</h3>${breakdownHtml(row=>row.method||'—')}</div></div>`;
}
