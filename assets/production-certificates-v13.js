...
function certificateFinanceRange(modeValue,yearValue,monthValue,dayValue){
  const financeMode=['daily','monthly','yearly'].includes(modeValue)?modeValue:'monthly',year=Number(yearValue||today().slice(0,4)),month=Math.max(1,Math.min(12,Number(monthValue||today().slice(5,7))));
...
function certificateFinanceBreakdown(rows,keyFn,total){
  const map=new Map();rows.forEach(row=>{const key=String(keyFn(row)||'—');map.set(key,(map.get(key)||0)+Number(row.amount||0));});
  const items=[...map.entries()].sort((a,b)=>b[1]-a[1]);if(!items.length)return'<div class="empty small">لا توجد بيانات</div>';
  return items.map(([name,value])=>{const pct=total?Math.min(100,Math.max(0,value/total*100)):0;return`<div class="break-row"><span>${esc(name)}</span><b>${cash(value)}</b><div><i style="width:${pct}%"></i></div></div>`;}).join('');
}
...
function drawCertificateFinance(){
  ...
  const modeValue=String(document.getElementById('certFinanceModeV13')?.value||'monthly'),day=document.getElementById('certFinanceDayV13')?.value||today(),year=document.getElementById('certFinanceYearV13')?.value||today().slice(0,4),month=document.getElementById('certFinanceMonthV13')?.value||today().slice(5,7),branch=document.getElementById('certFinanceBranchV13')?.value||'',method=document.getElementById('certFinanceMethodV13')?.value||'',range=certificateFinanceRange(modeValue,year,month,day);
  ...
  const rows=[...state.certificateReceipts].filter(receipt=>receipt.date>=range.from&&receipt.date<=range.to&&(!branch||certificateBranchKey(receipt)===branch)&&(!method||String(receipt.method||'')===method)).sort((a,b)=>b.date.localeCompare(a.date)||String(b.time||'').localeCompare(String(a.time||''))||Number(b.timestamp||0)-Number(a.timestamp||0));
  const total=rows.reduce((sum,row)=>sum+Number(row.amount||0),0),average=rows.length?Math.round(total/rows.length):0,biggest=Math.max(0,...rows.map(row=>Number(row.amount||0))),count=document.getElementById('certFinanceCountV13');if(count)count.textContent=`${rows.length} عملية`;
  ...
  root.innerHTML=`<div class="kpis"><div class="card"><small>دخل الشهادات</small><b>${cash(total)}</b><span>${esc(range.label)}</span></div><div class="card"><small>عدد العمليات</small><b>${rows.length}</b></div><div class="card"><small>متوسط العملية</small><b>${cash(average)}</b></div><div class="card"><small>أكبر عملية</small><b>${cash(biggest)}</b></div></div><div class="grid two breakdowns"><div class="card"><h3>حسب الفرع</h3>${certificateFinanceBreakdown(rows,row=>row.branchName||'—',total)}</div><div class="card"><h3>حسب وسيلة الدفع</h3>${certificateFinanceBreakdown(rows,row=>row.method||'—',total)}</div></div>${table(['رقم الروسي','الطالب','الفرع','الدورة','الوسيلة','التاريخ','المبلغ'],tableRows)}`;
...
function renderCertificates(){
  ...
  const branchOptions=state.certificateBranches.map(item=>`<option value="${esc(item.id)}">${esc(item.name)}</option>`).join(''),editable=canEditCertificates(),date=today(),currentYear=Number(date.slice(0,4)),currentMonth=Number(date.slice(5,7)),financeYears=certificateFinanceYears(),financeBranches=certificateFinanceBranches(),financeMethods=[...new Set(state.certificateReceipts.map(item=>String(item.method||'').trim()).filter(Boolean))];
  ...
  ['certFinanceModeV13','certFinanceDayV13','certFinanceMonthV13','certFinanceYearV13','certFinanceBranchV13','certFinanceMethodV13'].forEach(id=>document.getElementById(id)?.addEventListener('change',drawCertificateFinance));
