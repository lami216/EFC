...
function drawCertificateHistory(){
  ...
  const periodText=modeValue==='weekly'?`الفترة ${range.label}`:modeValue==='daily'?`يوم ${range.label}`:modeValue==='monthly'?`شهر ${range.label}`:range.label,branchLabel=branch?String(document.getElementById('certRecordsBranchV48')?.selectedOptions?.[0]?.textContent||'').trim():'',specialtyLabel=specialty?String(document.getElementById('certRecordsSpecialtyV48')?.selectedOptions?.[0]?.textContent||'').trim():'',methodLabel=method?String(document.getElementById('certRecordsMethodV48')?.selectedOptions?.[0]?.textContent||'').trim():'',filterParts=[periodText,branchLabel?`الفرع: ${branchLabel}`:'',specialtyLabel?`الشهادة / الدورة: ${specialtyLabel}`:'',methodLabel?`وسيلة الدفع: ${methodLabel}`:''].filter(Boolean);
  const caption=document.getElementById('certRecordsRangeV48');if(caption)caption.textContent=filterParts.join(' · ');
...
function renderStudentPicker(){
  ...
  root.innerHTML=!query?'<div class="cert-empty-v13">اكتب الاسم أو الهاتف أو رقم السجل، ويمكنك استخدام الفرع والدورة فقط لتضييق النتائج.</div>':matches.length?matches.map(item=>`<button class="cert-student-option-v13${String(student?.id)===String(item.id)?' is-selected':''}" data-student="${esc(item.id)}" type="button" aria-pressed="${String(student?.id)===String(item.id)?'true':'false'}"><b>${esc(item.name)}</b><span>${esc(item.phone||'—')} · سجل ${String(item.reg??'').padStart(4,'0')} · ${esc(branchName(item.branch))} · ${esc(spec(item.specialty)?.name||item.specialty||'—')}</span></button>`).join(''):'<div class="cert-empty-v13">لا يوجد طالب مطابق.</div>';
...
html body.efc-certificate-finance-open-v43 .cert-finance-topbar-v43{width:100%!important;display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;margin:0 0 14px!important;direction:rtl!important}
...
html body.efc-certificate-finance-open-v43 .cert-finance-controls-v13[data-mode="daily"]{grid-template-columns:154px 154px 110px 158px 120px!important;justify-content:start!important}
html body.efc-certificate-finance-open-v43 .cert-finance-controls-v13[data-mode="weekly"]{grid-template-columns:154px 145px 145px 110px 158px 120px!important;justify-content:start!important}
html body.efc-certificate-finance-open-v43 .cert-finance-controls-v13[data-mode="monthly"]{grid-template-columns:154px 96px 78px 110px 158px 120px!important;justify-content:start!important}
html body.efc-certificate-finance-open-v43 .cert-finance-controls-v13[data-mode="yearly"]{grid-template-columns:154px 84px 110px 158px 120px!important;justify-content:start!important}
...
html body .content:has(.cert-records-card-v47) .cert-records-topbar-v47{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;margin:0 0 8px!important;direction:rtl!important}
...
html body .content:has(.cert-records-card-v47) .cert-records-count-v47{min-width:340px!important;max-width:min(520px,62vw)!important;min-height:52px!important;height:auto!important;padding:7px 12px!important;border:1px solid #b9ddd5!important;border-radius:10px!important;background:linear-gradient(135deg,#eef9fb,#e8f5f2)!important;display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;direction:rtl!important}
html body .content:has(.cert-records-card-v47) .cert-records-count-v47>span{display:grid!important;gap:3px!important;min-width:0!important;flex:1 1 auto!important}html body .content:has(.cert-records-card-v47) .cert-records-count-v47 small{font-size:10px!important;font-weight:800!important;color:#35564d!important}html body .content:has(.cert-records-card-v47) .cert-records-count-v47 em{font-style:normal!important;font-size:8.5px!important;font-weight:700!important;line-height:1.35!important;color:#5f766e!important;white-space:normal!important;overflow-wrap:anywhere!important}html body .content:has(.cert-records-card-v47) .cert-records-count-v47 b{font-size:17px!important;color:#0a5f4d!important}
...
html body .content:has(.cert-records-card-v47) #certRecordsModeV48{height:34px!important;display:grid!important;grid-template-columns:repeat(4,1fr)!important;gap:3px!important;padding:3px!important;margin:0!important;border:1px solid #c9ddd6!important;border-radius:11px!important;background:#e9f4f0!important;align-self:end!important}
...
@media(max-width:940px){
  ...
  html body .content:has(.cert-records-card-v47) .cert-records-topbar-v47{flex-wrap:wrap!important}
  html body .content:has(.cert-records-card-v47) .cert-records-count-v47{width:100%!important}
...
@media(max-width:940px){
  ...
  html body.efc-certificate-finance-open-v43 .cert-finance-switch-v43{width:100%!important}
  html body.efc-certificate-finance-open-v43 .cert-finance-switch-v43 button{width:100%!important;min-width:0!important}
  html body.efc-certificate-finance-open-v43 .cert-finance-back-v43{max-width:100%!important}
...
@media(max-height:650px) and (max-width:1100px){
  ...
  html body.efc-certificate-finance-open-v43 .cert-finance-topbar-v43{margin-bottom:10px!important}
  html body.efc-certificate-finance-open-v43 .cert-finance-controls-v13{padding-top:6px!important;padding-bottom:7px!important;margin-bottom:6px!important}
