...
cert_path = Path("assets/production-certificates-v13.js")
...
cert = once(
    ...
    "receipt action styling",
)

receipt_document = r'''function receiptDocument(receipt,actions=true,autoPrint=false){
...
'''
cert = block(cert, "function receiptDocument(", "function openReceipt(", receipt_document, "certificate receipt actions")

helpers = r'''function resolveCertificateReceipt(reference){
...
'''
cert = once(cert, "function isOperationalStudent(student){", helpers + "\nfunction isOperationalStudent(student){", "certificate edit helpers")
...
cert = once(
    ...
    "edit-aware issue button",
)

finance_style = r'''function ensureCertificateFinanceStyles(){
...
html body.efc-certificate-finance-open-v43 .cert-finance-topbar-v43{width:100%!important;display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;margin:0 0 7px!important;direction:rtl!important}
...
'''
cert = block(cert, "function ensureCertificateFinanceStyles(){", "function bindCertificateFinanceRows(){", finance_style, "finance style v44")
...
cert = once(
    ...
    "finance period field visibility",
)

save_edit = r'''async function saveEditedReceipt(receipt){
...
'''
cert = once(cert, "async function issueReceipt(){", save_edit + "\nasync function issueReceipt(){", "edited receipt save flow")
cert = once(
    cert,
...
render_block = r'''function renderCertificates(){
...
'''
cert = block(cert, "function renderCertificates(){", "function ensureSidebar(){", render_block, "render certificates v44")

nav_guard = r'''document.addEventListener('click',event=>{
...
window.addEventListener('hashchange',()=>{if(editingReceiptId&&location.hash!=='#certificates')clearCertificateEdit();});
'''
cert = once(cert, "async function boot(){", nav_guard + "\nasync function boot(){", "certificate edit navigation guard")
...
cert = once(
    ...
    "certificate edit/delete public api",
)

cert = once(
    cert,
    "certificateFinanceTopbarAligned:true,certificateFinanceUsesGeneralChart:true,rollingFinancialYearsFrom2025:true",
    "certificateFinanceTopbarAligned:true,certificateFinanceUsesGeneralChart:true,certificateFinanceResponsive:true,certificateFinanceCompactSingleRowFilters:true,certificateFinanceResponsiveBreakdowns:true,certificateReceiptEditDelete:true,certificateReceiptAmountMethodOnlyEdit:true,certificateReceiptFiscalLockAware:true,rollingFinancialYearsFrom2025:true",
    "certificate capability markers",
)

cert_path.write_text(cert, encoding="utf-8")

verifier = Path("scripts/verify-stop-cert-finance-v17.mjs")
verify_text = verifier.read_text(encoding="utf-8")
old_line = "for(const [token,label] of [['certificateFinanceCurrentGeneralVisuals:true','current finance visual marker'],['certificateFinanceSummaryInControls:true','summary-in-controls marker'],['certificateFinanceTopbarAligned:true','aligned topbar marker'],['cert-finance-summary-v43','summary strip'],['certFinanceBackV13','finance back action'],['cert-finance-secondary-row-v43','method/summary row']])need(cert,token,label);"
new_line = "for(const [token,label] of [['certificateFinanceCurrentGeneralVisuals:true','current finance visual marker'],['certificateFinanceSummaryInControls:true','summary-in-controls marker'],['certificateFinanceTopbarAligned:true','aligned topbar marker'],['certificateFinanceResponsive:true','responsive finance marker'],['certificateFinanceCompactSingleRowFilters:true','compact filter marker'],['certificateFinanceResponsiveBreakdowns:true','responsive breakdown marker'],['certificateReceiptEditDelete:true','certificate receipt mutation marker'],['certificateReceiptAmountMethodOnlyEdit:true','amount/method-only edit marker'],['certificateReceiptFiscalLockAware:true','fiscal-lock mutation marker'],['cert-finance-summary-v44','summary strip'],['certFinanceBackV13','finance back action'],['EFC_EDIT_CERTIFICATE_RECEIPT_V13','receipt edit api'],['EFC_DELETE_CERTIFICATE_RECEIPT_V13','receipt delete api'],['EFC_CERTIFICATE_EDIT_V44','edit session api'],['grid-template-columns:154px 134px 86px 74px 110px minmax(130px,1fr) 112px','wide single-row filter grid'],['#certFinanceBodyV13>.breakdowns{grid-template-columns:1fr','small-screen breakdown stack'],['assertDateOpen','fiscal closed-period guard']])need(cert,token,label);"
verify_text = once(verify_text, old_line, new_line, "certificate verifier tokens")
verify_text = once(
    verify_text,
    ...
    "certificate verifier invariants",
)
...
verifier.write_text(verify_text, encoding="utf-8")

index = Path("index.html")
index_text = index.read_text(encoding="utf-8")
index_text = once(
    index_text,
    "        const editApi=window.EFC_REGISTRATION_EDIT_V17;\n        if(editApi?.active?.()&&!window.confirm('لديك تعديلات غير محفوظة. إغلاق البرنامج سيلغي التغييرات الحالية فقط. هل تريد المتابعة؟'))return;",
    "        const editApi=window.EFC_REGISTRATION_EDIT_V17,certificateEditApi=window.EFC_CERTIFICATE_EDIT_V44;\n        if((editApi?.active?.()||certificateEditApi?.active?.())&&!window.confirm('لديك تعديلات غير محفوظة. إغلاق البرنامج سيلغي التغييرات الحالية فقط. هل تريد المتابعة؟'))return;",
    "close guard for certificate edit",
)
index.write_text(index_text, encoding="utf-8")

old_version = "20260916-stop-end-cert-finance-v17-1"
new_version = "20260916-cert-management-responsive-v18-1"
version_files = [Path("index.html"), Path("assets/production-license-gate-v8.js")] + list(Path("scripts").glob("*.mjs"))
replaced = 0
for path in version_files:
    ...
    if count:
        ...
        replaced += count
if replaced < 4:
    raise SystemExit(f"cache version replacement count unexpectedly low: {replaced}")

print("Certificate management v18 patch applied.")
