...
for(const token of ['renderCurrentV13',"else if(page==='settings')renderSettings()","else if(page==='certificates')window.EFC_RENDER_CERTIFICATES_V13?.()",'canonicalSettingsRenderer:true','singleSettingsRenderOwner:true','noSettingsPostRenderEnhancement:true','settingsOwnedByFinalRouter:true','certificatesOwnedByFinalRouter:true','EFC_ENHANCE_FISCAL_SETTINGS_V14','loginAttemptThrottle:true','notificationBell:true','allPagesFinalRenderBeforeReveal:true','allPageRenderStaging:true','memoizedRouteReconcile:true','memoizedNotifications:true','singleAfterRenderPerRoute:true','efc-route-rendering'])requireText(securityUi,token,`security UI ${token}`);
